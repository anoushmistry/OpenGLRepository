...
int main()
{
  ...
  // ------------------------------------------------------------------
	...
	unsigned char* data1 = stbi_load("TextureTest.png", &width, &height, &nrChannels, 0); // 0 - No compression applied
	if (data1)
	{
	...
	// load image, create texture and generate mipmaps
	stbi_set_flip_vertically_on_load(true);
	unsigned char* data2 = stbi_load("TextureTest2.png", &width, &height, &nrChannels, 0); // 0 - No compression applied
	if (data2)
	{
